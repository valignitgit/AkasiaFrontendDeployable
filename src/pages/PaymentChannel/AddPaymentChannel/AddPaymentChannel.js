...
import { createPaymentChannel } from "redux/slices/paymentChannelSlice";
...
const AddPaymentChannel = () => {
  ...
  const [paymentChannelData, setPaymentChannelData] = useState(initialState);
    ...
    if (isValid) {
      try {
        const response = await dispatch(
          createPaymentChannel(paymentChannelData)
        ).unwrap();
        if (response.data) {
  ...
  const renderAddPaymentChannelDetailsForm = () => {
    return (
      <>
  ...
  return <>{renderAddPaymentChannelDetailsForm()}</>;
};
