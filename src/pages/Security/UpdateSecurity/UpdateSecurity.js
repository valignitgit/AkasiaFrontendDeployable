...
const AddSecurity = () => {
  ...
  const [currentSecurity, setCurrentSecurity] = useState(initialState);
  ...
  const onChange = (e) => {
    setCurrentSecurity({ ...currentSecurity, [e.target.name]: e.target.value });
  };
  const handleCheckboxChange = (e) => {
    const fieldName = e.target.name;
    const fieldValue = e.target.checked ? "yes" : "no";
    setCurrentSecurity({ ...currentSecurity, [fieldName]: fieldValue });
  };
